...
import { Bot, Send, Clock, Lightbulb, Loader2 } from "lucide-react";
...
const AIChatPanel = ({ city, zones }: AIChatPanelProps) => {
  ...
  return (
    <div className="glass-card flex flex-col h-[500px]">
      <div className="p-3 border-b border-border/50 flex items-center gap-2">
        ...
        <span className="text-sm font-semibold text-foreground">AI Pollution Analyst</span>
        <span className="text-xs text-muted-foreground ml-auto">Analyzing {city.name}</span>
      </div>
      ...
      <div className="p-2 border-b border-border/30 flex flex-wrap gap-1.5">
        ...
        <Button size="sm" variant="outline" className="text-xs h-7 gap-1" onClick={() => quickAction(`Predict pollution levels for ${city.name} in the next 24 hours.`)}>
          <Clock className="w-3 h-3" /> 24h Forecast
        </Button>
        <Button size="sm" variant="outline" className="text-xs h-7 gap-1" onClick={() => quickAction(`Provide policy insights for ${city.name}: What environmental measures have been tried before? Did they succeed or fail? If failed, suggest new alternatives.`)}>
      ...
      <ScrollArea className="flex-1 p-3" ref={scrollRef}>
        {messages.length === 0 && (
          <div className="text-center text-muted-foreground text-xs py-8 space-y-2">
            ...
            <p>Ask about pollution predictions, health advice, or policy insights for <strong>{city.name}</strong>.</p>
          </div>
        )}
